...
namespace NikosProGamer\MythicEffectClear;
...
class MythicEffectClear extends PluginBase implements Listener{
    
    public function onEnable()
    {
        $this->getLogger()->info("§d§lMEC §r§6Enabled!");
        $this->registerEvents();
            ...
            if ($sender->hasPermission("bless.cmd")) {
                foreach ($sender->getEffects() as $effect) {
                    if ($effect->getType()->isBad()) {
                        ...
                        $sender->sendMessage("§cYou do not have permission to use §d§lMEC§r§c! Buy a §6§lRank§r §cto gain access to it!");                 
